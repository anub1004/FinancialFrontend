...
function computeHealthScore(s: DashboardSummary | null): {
  ...
  label: string;
  color: string;
  signals: { name: string; value: number; color: string }[];
} {
  if (!s) return { score: 0, label: "No Data", color: "#9ca3af", signals: [] };

  // 1. Savings Rate (35%) — savingsRate is 0–100 from API; we map 50%+ → 100 score
  const savingsRaw = Math.min(Math.max(s.savingsRate, 0), 50);
  const savingsScore = (savingsRaw / 50) * 100;

  // 2. Net Balance Positivity (25%)
  const netScore =
    s.netBalance >= 0
      ? Math.min(
          100,
          60 + (s.totalIncome > 0 ? (s.netBalance / s.totalIncome) * 40 : 0),
        )
      : Math.max(
          0,
          30 + (s.totalExpense > 0 ? (s.netBalance / s.totalExpense) * 30 : 0),
        );

  // 3. Investment Health (20%)
  let investScore = 0;
  if (s.totalInvested > 0) {
    investScore = 60; // base for having investments
    if (s.investmentReturns > 0) {
      investScore += Math.min(
        40,
        (s.investmentReturns / s.totalInvested) * 200,
      );
    } else if (s.investmentReturns < 0) {
      investScore = Math.max(
        20,
        60 + (s.investmentReturns / s.totalInvested) * 100,
      );
    }
  }
  // 4. Goal Progress (20%)
  const goalScore =
    (s.goalsTotalTarget ?? 0) > 0
      ? Math.min(
          100,
          ((s.goalsTotalSaved ?? 0) / (s.goalsTotalTarget ?? 1)) * 100,
        )
      : (s.activeGoals ?? 0) > 0
        ? 30
        : 0;
  ...
  const score = Math.round(
    savingsScore * 0.35 + netScore * 0.25 + investScore * 0.2 + goalScore * 0.2,
  );
  ...
  return {
    ...
    color,
    signals: [
      { name: "Savings", value: Math.round(savingsScore), color: "#8b5cf6" },
      { name: "Balance", value: Math.round(netScore), color: "#0ea5e9" },
      { name: "Investments", value: Math.round(investScore), color: "#f59e0b" },
      { name: "Goals", value: Math.round(goalScore), color: "#10b981" },
    ],
  };
}
...
function FinancialHealthScore({
  ...
  const { score, label, color, signals } = computeHealthScore(summary);
  ...
  const dashOffset = circumference - (animatedDash / 100) * circumference;
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      const [sumRes, trendRes, catRes, actRes] = await Promise.all([
        fetch(`${ApiConfig.Api_Base_Url}api/dashboard/summary`, {
          credentials: "include",
          headers,
        }),
        fetch(`${ApiConfig.Api_Base_Url}api/dashboard/monthly-trend`, {
          credentials: "include",
          headers,
        }),
        fetch(`${ApiConfig.Api_Base_Url}api/dashboard/category-breakdown`, {
          credentials: "include",
          headers,
        }),
        fetch(`${ApiConfig.Api_Base_Url}api/dashboard/recent-activity`, {
          credentials: "include",
          headers,
        }),
      ]);
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              label: (ctx: any) =>
                ` ₹${ctx.parsed.toLocaleString("en-IN")} (${categories[ctx.dataIndex]?.percentage ?? 0}%)`,
            },
          },
        },
      },
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
      ...
      >
        <SummaryCard
          ...
          label="Income"
          value={fmt(summary?.totalIncome ?? 0, summary?.currency)}
        ...
        <SummaryCard
          ...
          label="Expenses"
          value={fmt(summary?.totalExpense ?? 0, summary?.currency)}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-lg font-bold text-gray-800 dark:text-gray-100">
                    {(summary?.goalsTotalTarget ?? 0) > 0
                      ? Math.round(
                          ((summary?.goalsTotalSaved ?? 0) /
                            (summary?.goalsTotalTarget ?? 1)) *
                            100,
                        )
                      : 0}
                    %
      ...
      {summary && <FinancialHealthScore summary={summary} />}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden mb-8">
        ...
        {activities.length > 0 ? (
            ...
            {activities.map((a) => (
              ...
              >
                ...
                <div className="text-right">
                  <div
                    className={`text-sm font-semibold ${a.isPositive ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}
                  >
                    {a.isPositive ? "+" : "−"}
          ...
          <div className="px-6 py-12 text-center text-gray-400 text-sm">
            No recent activity. Start adding transactions, investments, or
            goals!
          </div>
        )}
      </div>
        ...
        <FeatureGate
          ...
          fallback={
            <QuickAction
              ...
              title="Investments 🔒"
              desc="Upgrade to Basic+ to unlock"
              color="sky"
            />
          }
        >
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div
            className={`text-lg font-bold ${valueColor || "text-gray-800 dark:text-gray-100"}`}
          >
            {value}
          </div>
          {subtext && (
            <div
              className={`text-xs mt-0.5 ${subtextColor || "text-gray-400"}`}
            >
              {subtext}
            </div>
          )}
...
function QuickAction({
  ...
  const colors: Record<string, string> = {
    violet:
      "bg-violet-50 dark:bg-violet-900/10 border-violet-100 dark:border-violet-800 text-violet-600 dark:text-violet-400 hover:bg-violet-100 dark:hover:bg-violet-900/20",
    ...
    emerald:
      "bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-800 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/20",
  };
  return (
    <NavLink
      to={to}
      className={`flex items-center gap-4 p-5 rounded-2xl border shadow-sm transition-colors ${colors[color] || colors.violet}`}
    >
