...
export default function PlanManagement({
  ...
  const fetchPlans = useCallback(async () => {
    try {
      const res = await fetch(
        ApiConfig.Api_Base_Url + "api/admin/plans?includeInactive=true",
  ...
  const fetchFeatures = useCallback(async () => {
    try {
      const res = await fetch(ApiConfig.Api_Base_Url + "api/admin/features", {
        credentials: "include",
        headers,
      });
  ...
  useEffect(() => {
    Promise.all([fetchPlans(), fetchFeatures()]).finally(() =>
      setLoading(false),
    );
  }, []);
    ...
    try {
      ...
      if (!editingPlan) {
        for (const fid of selectedFeatures) {
          await fetch(
            ApiConfig.Api_Base_Url + `api/admin/plans/${data.id}/features`,
            {
              method: "POST",
              credentials: "include",
              headers,
  ...
  const deletePlan = async (id: string) => {
    ...
    try {
      const res = await fetch(
        ApiConfig.Api_Base_Url + `api/admin/plans/${id}`,
  ...
  const assignFeature = async (planId: string, featureId: string) => {
    try {
      const res = await fetch(
        ApiConfig.Api_Base_Url + `api/admin/plans/${planId}/features`,
        {
          method: "POST",
          credentials: "include",
          headers,
    ...
    try {
      const res = await fetch(
        ApiConfig.Api_Base_Url +
          `api/admin/plans/${planId}/features/${featureId}`,
        {
          method: "DELETE",
          credentials: "include",
          headers,
  ...
  const savePricing = async () => {
    ...
    try {
      const res = await fetch(
        ApiConfig.Api_Base_Url + `api/admin/plans/${pricingPlanId}/pricing`,
        {
          method: "PUT",
          credentials: "include",
          headers,
          body: JSON.stringify({
            ...
            annualPrice: Number(pricingForm.annualPrice) || null,
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-7xl mx-auto">
      ...
      <div className="sm:flex sm:justify-between sm:items-center mb-8">
        ...
        <button
          onClick={openCreate}
          className="mt-4 sm:mt-0 btn bg-violet-600 hover:bg-violet-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-sm"
        >
          + New Plan
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {plans.map((p) => (
          <div
            key={p.id}
            className={`bg-white dark:bg-gray-800 rounded-xl border ${p.isActive ? "border-gray-200 dark:border-gray-700" : "border-red-200 dark:border-red-800 opacity-70"} shadow-sm overflow-hidden`}
          >
            <div className="p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-1">
                  ...
                  <span
                    className={`px-2 py-0.5 text-xs font-medium rounded-full ${p.isActive ? "bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300" : "bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300"}`}
                  >
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {p.features.map((f) => (
                    <span
                      key={f.id}
                      className="inline-flex items-center gap-1 px-2 py-0.5 text-xs bg-violet-50 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 rounded-full"
                    >
                      {f.displayName}
                      <button
                        onClick={() => removeFeature(p.id, f.id)}
              ...
              <div className="mb-4">
                <select
                  ...
                  defaultValue=""
                  onChange={(e) => {
                    if (e.target.value) assignFeature(p.id, e.target.value);
                    e.target.value = "";
                  }}
                >
                  ...
                  {features
                    .filter(
                      (f) =>
                        f.isActive && !p.features.some((pf) => pf.id === f.id),
                    )
                    .map((f) => (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border border-gray-200 dark:border-gray-600 dark:bg-gray-700 rounded-lg px-3 py-2 text-sm"
              ...
              <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
                ...
                <button
                  onClick={savePlan}
                  className="px-4 py-2 text-sm font-medium text-white bg-violet-600 hover:bg-violet-700 rounded-lg shadow-sm"
                >
