...
function Signup() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await axios.post(ApiConfig.Api_Base_Url + "api/Auth/register", data, { withCredentials: true });
      const d = res.data;
      if (d.emailOtpRequired) {
        setEmailOtpData({ active: true, email: data.email, totpSessionToken: d.totpSessionToken });
        setEmailTimer(120);
        toast.success("Account created! Verify your email address to continue.");
      } else if (d.totpRequired) {
        setTotpData({ active: true, setupRequired: d.totpSetupRequired, qr: d.qrCodeBase64 || "", key: d.manualEntryKey || "", session: d.totpSessionToken || "", email: data.email });
  ...
  const handleEmailOtpSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      setVerifying(true);
      const result = await verifySignupEmailOtp(emailOtpData.email, emailOtpCode);
      setTotpData({
        active: true,
            ...
            <div className={styles.contentWrap}>
              ...
              <form className={styles.form} onSubmit={handleTotpSubmit}>
                <label className={styles.field}>
                  ...
                  <input type="text" maxLength={6} placeholder="000000" value={totpCode}
